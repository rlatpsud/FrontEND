    ...
    function fillData(data){
        ...
        new Chart(document.getElementById("radar-chart"), {
            type: 'radar',
            data: {
              labels: ['평균',"정확도", "유창성", "문장 완성도", "발음 정확도"],
              datasets: [
                {
                  label: "결과",
                  fill: true,
                  ...
                  pointBackgroundColor: "rgba(30, 22, 54,1)",
                  data: [(data.AccuracyScore+data.FluencyScore+data.CompletenessScore+(parseInt(data.PronScore, 10)))/4,
                  data.AccuracyScore,data.FluencyScore,data.CompletenessScore,parseInt(data.PronScore, 10)]
                    
                // data: [data.AccuracyScore,data.AccuracyScore,data.AccuracyScore,data.AccuracyScore,data.AccuracyScore]
                }
              ]
            },
            options: {
        ...
        if ((data.AccuracyScore+data.FluencyScore+data.CompletenessScore+(parseInt(data.PronScore, 10)))/4 >=90)
            score.innerText = 'A',
            rank.innerText = 'Rank',
            comment.innerText = '당신은 이미 원어민 :)'
        else if ((data.AccuracyScore+data.FluencyScore+data.CompletenessScore+(parseInt(data.PronScore, 10)))/4 >=80)
            score.innerText = 'B',
            rank.innerText = 'Rank',
            comment.innerText = '야 너도 할 수 있어.'
        else if ((data.AccuracyScore+data.FluencyScore+data.CompletenessScore+(parseInt(data.PronScore, 10)))/4 >=70)
            score.innerText = 'C',
            rank.innerText = 'Rank',
            comment.innerText = '조금만 더 힘을 내보아요~~ㅎㅎ'
        else if ((data.AccuracyScore+data.FluencyScore+data.CompletenessScore+(parseInt(data.PronScore, 10)))/4 >=60)
            score.innerText = 'D',
            rank.innerText = 'Rank',
            comment.innerText = '학창시절에 Dㅣ지게 공부안하셨네요..'
        else if ((data.AccuracyScore+data.FluencyScore+data.CompletenessScore+(parseInt(data.PronScore, 10)))/4 < 60)
            score.innerText = 'F',
            rank.innerText = 'Rank',
            comment.innerText = '당신은 우리말 지킴이 :(';
    }
